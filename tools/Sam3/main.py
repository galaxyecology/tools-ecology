...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Comma-separated list of class text prompts (e.g., 'human,elephant')"
    )
    parser.add_argument(
        "--conf",
        type=float,
...
def create_coco_categories(text_prompts: List[str]) -> List[Dict[str, Any]]:
    ...
    """
    return [{"id": i + 1, "name": label} for i, label in enumerate(text_prompts)]


def create_coco_output(results: List[Any], text_prompts: List[str], metadata: Dict[str, Any]) -> Dict[str, Any]:
...
def create_yolo_output(annotation_type: str, results: List[Any], output_dir: Path) -> None:
    ...
    for result in results:
        ...
        for i, (box, class_id) in enumerate(zip(result.boxes.xyxyn, result.boxes.cls)):
...
def create_yolo_video_output(annotation_type: str, results: List[Any], output_dir: Path, video_path: str, stride: int) -> None:
...
def create_metadata(text_prompts: List[str], conf_threshold: float , model_path: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "model": "sam3.pt",
        "model_sha256": compute_file_hash(model_path) if model_path.exists() else "N/A",
...
def main():
    ...
    if "yolo_bbox" in output_formats:
        ...
        if is_video(file_paths[0]):
            create_yolo_video_output("bbox", results, yolo_bbox_dir, file_paths[0], args.vid_stride)
        else:
    ...
    if "yolo_seg" in output_formats:
        ...
        if is_video(file_paths[0]):
            create_yolo_video_output("seg", results, yolo_seg_dir, file_paths[0], args.vid_stride)
        else:
