...
echo -e "Dataset\tN_SNPs_before\tN_SNPs_after_mD" >"${summary_file}"

######################################################
#Function: vcf_filtering_SNP_missingdata_MAC
#Description: Remove SNPs with a lot of missing data
#######################################################
...
vcf_filtering_SNP_missingdata(){
    ...
    local MAX_MISSING_LOCI="$3"
        ...
        output_file_gz="${vcf_dir}/${base_name}_SNPmd.vcf.gz" #Final output file - gzip
        output_file="${vcf_dir}/${base_name}_SNPmd.vcf" #Final output file

        bcftools filter -e "F_MISSING > ${MAX_MISSING_LOCI}" -O z -o "$output_file_gz" "$vcf"

        if [[ ! -f "$output_file_gz" ]]; then
            echo "File output not found"
            return
        fi
...
vcf_filtering_SNP_missingdata "$vcf_input" "$vcf_names" "$MAX_MISSING_LOCI"