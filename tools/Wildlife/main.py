...
taxons = (
    list(labels.values()) if type_mapping.lower() == "id2label" else list(labels.keys())
)
...
def predict_images(images_dir, detections_dir, predictions, boxing_mode):
    ...
    detections_list = detection_model.batch_image_detection(
        data_path=images_dir, batch_size=16, det_conf_thres=detection_threshold  # 32
    )
    ...
    for detection in tqdm(
        detections_images, desc="Classifying...", unit="step", colour="yellow"
    ):
        ...
        filepath = filename
        print(
            f"Traitement de la détection {detection} de classe {detection_class} avec score {detection_score}, xyxy {xyxy}"
        )
...
for filepath in tqdm(
    input_files, desc="Extracting frames...", unit="file", colour="green"
):
    ...
    if images_count > images_max:
        predictions = predict_images(
            images_dir, detections_dir, predictions, boxing_mode=boxing_mode
        )
        images_count = 0
    ...
    if mime.startswith("image"):
        frame_idx = 0
        shutil.copy(
            str(filepath), os.path.join(images_dir, f"F{frame_idx}_{filename}.JPG")
        )
        images_count += 1
    else:
...
if "Frame" in predictions_csv.columns:
    predictions_csv = predictions_csv.sort_values(by=["Filename", "Frame"]).reset_index(
        drop=True
    )
else:
...
for (filename, species), df_group in predictions_valid.groupby(
    ["Filename", "Prediction"]
):
...
predictions_recap = predictions_recap.sort_values(
    by=["Filename", "Species"]
