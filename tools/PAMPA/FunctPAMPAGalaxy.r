...
######################################### start of the function drop_levels_f called y calc_biodiv_f in FucntExeCalcCommIndexesGalaxy.r and linear_model_wp2_community_f in FunctExeCalcGLMGalaxy.r
...
######################################### start of the function subset_all_tables_f called by linear_model_wp2_community_f in FunctExeCalcGLMGalaxy.r
...
######################################### start of the function sorties_lm_f called by linear_model_wp2_community_f in FunctExeCalcGLMGalaxy.r
                        ...
                        Log=FALSE, sufixe=NULL)
{
    ...
    if (length(grep("^glmmTMB", obj_lm$call)) > 0) #if random effects
    {
        ...
        if (! is.null(lev)) ## if fixed effects + random effects
        {
            ...
            tab_coef$signif <- lapply(tab_coef[,"Pr(>|z|)"],FUN=function(x){if(!is.na(x) && x < 0.05){"yes"}else{"no"}})
            ...
            tab_sum[tab_sum[,col_ana]==cut,grepl(paste(lev,"Zvalue",collapse="|"),colnames(tab_sum))] <- unlist(lapply(lev,FUN=function(x){if (length(grep(x,rownames(tab_coef))) > 0) {tab_coef[grepl(x,rownames(tab_coef)),"z value"]}else{NA}}))
            tab_sum[tab_sum[,col_ana]==cut,grepl(paste(lev,"Pvalue",collapse="|"),colnames(tab_sum))] <- unlist(lapply(lev,FUN=function(x){if (length(grep(x,rownames(tab_coef))) > 0) {tab_coef[grepl(x,rownames(tab_coef)),"Pr(>|z|)"]}else{NA}}))

            if (any(obj_lmy != ""))
            {
                sum_lmy <- summary(obj_lmy)
                tab_coefy <- as.data.frame(sum_lmy$coefficients$cond)
                tab_coefy$signif <- lapply(tab_coefy[,"Pr(>|z|)"],FUN=function(x){if(!is.na(x) && x < 0.05){"yes"}else{"no"}})
        ...
        if (sum_lm$family[1] == "gaussian" || sum_lm$family[1] == "quasipoisson") 
        {
            ...
            tab_coef$signif <- lapply(tab_coef[,"Pr(>|t|)"],FUN=function(x){if(!is.na(x) && x < 0.05){"yes"}else{"no"}})
            ...
            tab_sum[tab_sum[,col_ana]==cut,grepl(paste(lev,"Tvalue",collapse="|"),colnames(tab_sum))] <- unlist(lapply(lev,FUN=function(x){if (length(grep(x,rownames(tab_coef))) > 0) {tab_coef[grepl(x,rownames(tab_coef)),"t value"]}else{NA}}))

            tab_sum[tab_sum[,col_ana]==cut,grepl(paste(lev,"Pvalue",collapse="|"),colnames(tab_sum))] <- unlist(lapply(lev,FUN=function(x){if (length(grep(x,rownames(tab_coef))) > 0) {tab_coef[grepl(x,rownames(tab_coef)),"Pr(>|t|)"]}else{NA}}))

            if (any(obj_lmy != ""))
            {
                tab_coefy$signif <- lapply(tab_coefy[,"Pr(>|t|)"],FUN=function(x){if(!is.na(x) && x < 0.05){"yes"}else{"no"}})
    ...
    if (! is.null(lev)) ## if fixed effects
    {
        ...
        tab_sum[tab_sum[,col_ana]==cut,grepl(paste(lev,"Estimate",collapse="|"),colnames(tab_sum))] <- unlist(lapply(lev,FUN=function(x){if (length(grep(x,rownames(tab_coef))) > 0) {tab_coef[grepl(x,rownames(tab_coef)),"Estimate"]}else{NA}}))
        tab_sum[tab_sum[,col_ana]==cut,grepl(paste(lev,"Std.Err",collapse="|"),colnames(tab_sum))] <- unlist(lapply(lev,FUN=function(x){if (length(grep(x,rownames(tab_coef))) > 0) {tab_coef[grepl(x,rownames(tab_coef)),"Std. Error"]}else{NA}}))
        tab_sum[tab_sum[,col_ana]==cut,grepl(paste(lev,"signif",collapse="|"),colnames(tab_sum))] <- unlist(lapply(lev,FUN=function(x){if (length(grep(x,rownames(tab_coef))) > 0) {tab_coef[grepl(x,rownames(tab_coef)),"signif"]}else{NA}})) 
    ...
    tab_sum[tab_sum[,col_ana]==cut,grepl(paste(lev,"IC_up",collapse="|"),colnames(tab_sum))] <- unlist(lapply(lev,FUN=function(x){if (length(grep(x,rownames(ic))) > 0) {ic[grepl(x,rownames(ic)),"97.5 %"]}else{NA}})) 
    tab_sum[tab_sum[,col_ana]==cut,grepl(paste(lev,"IC_inf",collapse="|"),colnames(tab_sum))] <- unlist(lapply(lev,FUN=function(x){if (length(grep(x,rownames(ic))) > 0) {ic[grepl(x,rownames(ic)),"2.5 %"]}else{NA}})) 
...
######################################### start of the function note_glm_f called by linear_model_wp2_species_f and linear_model_wp2_community_f
...
######################################### start of the function note_glms_f called by linear_model_wp2_species_f and linear_model_wp2_community_f
...
######################################### start of the function info_stats_f called by linear_model_wp2_species_f and linear_model_wp2_community_f
