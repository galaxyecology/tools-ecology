                       ...
                       trendOnGraph=TRUE,assessIC=TRUE) 
{
    ...
    switch(as.character(metric), ## Different value represented graph 2 depending on the nature of the metric
           ...
           "pres.abs" = {nb_loc <- lapply(sort(year), FUN=function(x){length(unique(subset(datatablecut,year == x)[,"location"]))}) ## number of plots per year
...
#plot <- rbind_ggplot_timeseries(ggplot_list=plots, limits = c(dmy("2008","2017"))) ##Bind several timeseries plots onto one : to investigate
